...
class InterestsSection(SectionView):
    def __init__(self, parent, data=None):
        super().__init__(parent, "interests")
        self.container = ttk.Frame(self.frame)
        self.container.pack(fill="x")
        self.add_button = ttk.Button(
            self.frame, text="Add Interest", command=self.add_item, bootstyle=SUCCESS
        )
    ...
    def add_item(self):
        ...
        interest_entry = ttk.Entry(item_frame, font=FONTS["mono"])
        interest_entry.grid(row=0, column=0, sticky="ew", padx=(5, 5))
        remove_btn = ttk.Button(
            item_frame,
            text="Remove",
            command=lambda: self.remove_item(
                item_frame, {"frame": item_frame, "interest": interest_entry}
            ),
            bootstyle=DANGER,
        )
    ...
    def load_data(self, data):
        for item in data:
            if isinstance(item, str):
                self.add_item()
                last = self.items[-1]
                last["interest"].delete(0, tk.END)
                last["interest"].insert(0, item)

    def get_data(self):
        return [{"left": True}] + [item["interest"].get() for item in self.items]
